// Bloom in Oklab perceptual color space — no chromatic aberration, no animation.
// Based on glow-rgbsplit-twitchy.glsl's glow approach, ~25% lighter than bloom.glsl.
...
vec4 toRgb(vec4 oklab) {
    ...
    return vec4(clamp(f(lin.r), 0.0, 1.0), clamp(f(lin.g), 0.0, 1.0), clamp(f(lin.b), 0.0, 1.0), oklab.a);
}
