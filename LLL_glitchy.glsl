...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // per-scanline horizontal warp — scales with glitch, constants keep a subtle baseline drift
    float y = uv.y * iResolution.y;
    float distortion = gnoise(vec3(0., y * .01, t * 500.)) * (glitchAmount * 4.);
    distortion *= gnoise(vec3(0., y * .02, t * 250.)) * (glitchAmount * 2.);
    // perpetual baseline horizontal vibration:
    // float distortion = gnoise(vec3(0., y * .01, t * 500.)) * (glitchAmount * 4. + .1);
    // distortion *= gnoise(vec3(0., y * .02, t * 250.)) * (glitchAmount * 2. + .025);

    ++displayNoise;
    // thin fast bands, gated to fire briefly every ~40s and ~60s
    float gate1 = SS(0., 1.5, mod(t, 40.));
    float gate2 = SS(0., 1.5, mod(t, 60.));
    float scanline = smoothstep(.999, 1., sin((uv.y + t * 1.76) * 2.)) * gate1
                   - smoothstep(.999, 1., sin((uv.y + t * 1.1) * 2.)) * gate2;
    distortion += scanline * .08;
    st = uv + vec2(distortion, 0.);

    // chromatic aberration — in scan line bands always, full aberration during glitch
    vec2 aberration = (eps + distortion) * glitchAmount + eps * abs(scanline) * 12.;
    col.r += textureLod(iChannel0, st + aberration, 0.).r;
    col.g += textureLod(iChannel0, st, 0.).g;
    col.b += textureLod(iChannel0, st - aberration, 0.).b;
    ...
    // white noise + scanlines (only during glitch)
    displayNoise = 0.2 * clamp(displayNoise, 0., 1.) * glitchAmount;
